...
module.exports = (function ($) {
  ...
  $(document).ready(function() {
    ...
    function scroll (e) {
      ...
      $("body, html").animate({ scrollTop: e.data.scrollTop }, scrollTime);
    }

    $('.menu-desktop')
        .on('click', '.menu-desktop-projects a', {scrollTop : pageHeight}, scroll)
        .on('click', '.menu-desktop-skills a', {scrollTop : pageHeight * 2}, scroll);

    $('.hamburger-toggle .menu-list')
      .on('click', '.projects-link', {scrollTop : pageHeight}, scroll)
      .on('click', '.skills-link', {scrollTop : pageHeight * 2}, scroll);

    $('.top-button')
      .on('click', '.link-to-top', {scrollTop : 0}, scroll);
    ...
    function projectsNavigationHandler(e) {
      $(this).css({ "background-color" : e.data.bgcolor, "color" : e.data.color});
    }

    $('.project-navigation')
      .on('mouseover', '.project-prev', { bgcolor : "#3fe6ff", color : "#fff"}, projectsNavigationHandler)
      .on('mouseout','.project-prev', { bgcolor : "", color : ""}, projectsNavigationHandler)
      .on('mouseover', '.project-next', { bgcolor : "#ee587b", color : "#fff"}, projectsNavigationHandler)
      .on('mouseout','.project-next', { bgcolor : "", color : ""}, projectsNavigationHandler);
    ...
    $('.skills-content')
      .on('mouseover', '.elementary', {text: 'elementary'}, showLevel)
      .on('mouseover', '.moderate', {text: 'moderate'}, showLevel)
      .on('mouseover', '.advanced', {text: 'advanced'}, showLevel)
